...
export default function UserSecurityModal({ isOpen, onClose }) {
  ...
  const onSubmitPassword = (e) => {
    ...
    try {
      ...
      setPassSuccess('密码已成功重置修改！新密码即刻生效。');
      setOldPassword('');
      setNewPassword('');
      setConfirmPassword('');
      if (showToast) showToast('主守护密码已成功修改', 'success');
    } catch (err) {
          ...
          <form onSubmit={onSubmitProfile} className="space-y-4">
            ...
            <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1.5">用户姓名 / 昵称</label>
                  <input
                    type="text"
                    value={userName}
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1.5">绑定的邮箱账号</label>
                  <input
                    type="email"
                    value={userEmail}
